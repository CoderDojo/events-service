...
describe('integration:tickets', () => {
  ...
  it('should return the tickets of a session', async () => {
    ...
    expect(res.body.results[1].id).to.equal('58544293-9d1e-4ae0-b061-e005225886b2');
    expect(res.body.results[1].sessionId).to.equal('e688e464-db01-42fa-b655-5d93fadc3ed8');
    expect(res.body.results[1].name).to.equal('Scratch');
    expect(res.body.results[1].type).to.equal('ninja');
    expect(res.body.results[1].quantity).to.equal(42);
